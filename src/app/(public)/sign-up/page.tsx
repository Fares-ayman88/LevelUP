...
function googleErrorMessage(error: string | string[] | undefined): string | undefined {
  ...
  if (code === "google_access_denied") return "Google sign-up was cancelled. Try again when you are ready.";
  if (code === "google_not_configured") return "Google sign-up is not configured for this workspace yet.";
  if (code === "google_unavailable") return "This Google account is not available for LevelUp right now.";
  if (code === "google_failed") return "Google sign-up could not be completed. Please try again.";
  return undefined;
}
  ...
  return (
    <main className="relative isolate min-h-screen overflow-hidden bg-[#07090d] px-5 py-5 text-white sm:px-8 sm:py-8">
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-2.5rem)] w-full max-w-6xl flex-col">
        ...
        <div className="my-auto grid items-center gap-10 py-10 lg:grid-cols-[minmax(0,1fr)_540px] lg:gap-20">
          <section className="max-w-2xl">
            ...
            <h1 className="mt-5 max-w-xl text-4xl font-semibold leading-[1.06] text-white sm:text-6xl">
              One account for every learning day.
            </h1>
            <p className="mt-6 max-w-lg text-base leading-7 text-slate-300 sm:text-lg">
              Create your account first, then connect it to the right center without mixing access between centers.
            </p>
          </section>
